...
function StripSection({
  ...
  return (
    <section
      className={cn(
        "flex h-full min-h-[148px] flex-col rounded-[16px] border border-[var(--line)]/65 bg-[var(--surface-0)]/58 px-4 py-3 sm:rounded-[22px] sm:px-5 sm:py-4",
        className,
      )}
    >
...
export function HomeStage({
  ...
  return (
    <Card className="tp-enter overflow-hidden border-[color:var(--accent)]/10 bg-[var(--surface-0)] p-0 shadow-[0_28px_72px_rgba(0,0,0,0.16)]">
      {/* One wrapper, not two: the decorative gradient below gets a negative
          z-index so it paints behind this content without needing a second
          "position: relative" layer just to win stacking order. */}
      <div className="relative isolate overflow-hidden rounded-[26px] px-4 py-5 sm:rounded-[34px] sm:px-7 sm:py-7 xl:px-8 xl:py-8">
        <div className="pointer-events-none absolute inset-0 -z-10 bg-[linear-gradient(135deg,rgba(62,209,189,0.08),transparent_38%,rgba(247,183,86,0.04))]" />
        ...
        <div className="mt-5 grid gap-6 xl:grid-cols-[minmax(0,1.34fr)_minmax(300px,0.66fr)] xl:items-stretch">
          <div className="min-w-0">
            ...
            <p className="mt-4 max-w-[68ch] text-sm leading-7 text-[var(--muted)] sm:text-base">
              Pick the working style, decide whether you want a target, then add audio. The results view opens as soon as you add files, with the table, charts, and file details laid out at full width.
            </p>
          </div>

          <div className="flex min-h-[178px] h-full flex-col justify-between rounded-[24px] border border-[var(--line)]/60 bg-[var(--surface-1)]/38 px-4 py-4 text-sm leading-6 text-[var(--muted)] sm:px-5 sm:py-5">
        ...
        <div className="mt-6 rounded-[22px] border border-[var(--line)]/72 bg-[var(--surface-1)]/52 px-4 py-5 sm:mt-7 sm:rounded-[30px] sm:px-7 sm:py-7">
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)] xl:items-stretch">
            <section
              aria-label="Drag and drop area"
              className={cn(
                "rounded-[18px] border border-dashed px-4 py-4 transition-[background-color,border-color,box-shadow] duration-200 ease-out sm:rounded-[24px] sm:px-5 sm:py-5",
                isDragging
            ...
            >
              ...
              <h3 className="mt-3 text-balance text-2xl font-semibold text-[var(--ink)] sm:text-3xl">
                {isCoarsePointer ? "Drop files anywhere" : "Drop files or folders anywhere"}
              </h3>
              <p className="mt-3 max-w-[60ch] text-sm leading-7 text-[var(--muted)] sm:text-base">
                {isCoarsePointer
                  ? "Drag audio files onto this page, or use the file picker below to choose files from your device."
                  : "Drag a batch, or a whole album folder, onto any part of the window. Folders are scanned for supported audio, and the session opens once the files are accepted."}
              </p>
              <div className="mt-5 inline-flex items-center gap-2 rounded-full border border-[var(--line)]/70 bg-[var(--surface-0)]/54 px-3 py-1.5 text-[11px] uppercase tracking-[0.18em] text-[var(--muted)]">
                ...
                {isCoarsePointer ? "Works on every screen" : "Works on every screen, folders included"}
              </div>
            </section>
            ...
            <div className="flex min-h-[168px] h-full flex-col justify-between gap-4 xl:pl-2">
              <div>
                ...
                <p className="mt-3 max-w-[34ch] text-sm leading-6 text-[var(--muted)]">
                  Prefer to browse? The button below opens the file picker and moves straight to the review screen.
                </p>
              </div>
              <Button
                type="button"
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4 xl:items-stretch">
            <StripSection label="Workspace mode">
              <div className="flex h-full flex-col justify-between gap-4">
                <div className="grid grid-cols-2 gap-2 rounded-[18px] border border-[var(--control-line)] bg-[var(--surface-1)]/78 p-1" role="group" aria-label="Choose workspace mode">
            ...
            <StripSection label="Analysis mode">
              <div className="flex h-full flex-col justify-between gap-4">
                <div className="grid grid-cols-2 gap-2 rounded-[18px] border border-[var(--control-line)] bg-[var(--surface-1)]/78 p-1" role="group" aria-label="Choose analysis mode">
            ...
            <StripSection label="Advanced" className="justify-between">
              <div className="flex h-full flex-col justify-between gap-4">
                <Button
                  ...
                  size="sm"
                  variant="ghost"
                  className="h-11 w-full justify-between rounded-[18px] border border-[var(--control-line)] bg-[var(--surface-0)]/58 px-4 text-[var(--ink)]"
