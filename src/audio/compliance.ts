...
export function getComplianceSummary(result: AnalysisResult): ComplianceSummary | null {
  ...
  // Compliance uses the same 0.01 precision as the UI and reports. Round each
  // displayed operand first so a verdict cannot disagree with equal-looking
  // values at a tolerance or true-peak boundary.
  const measuredIntegratedLufs = roundToMeasurementPrecision(result.metrics.integratedLufs);
  const targetLoudnessLufs = roundToMeasurementPrecision(result.target.loudnessTargetLufs);
  const deltaFromTargetLufs = roundToMeasurementPrecision(
    measuredIntegratedLufs - targetLoudnessLufs,
  );
  const window = roundToMeasurementPrecision(result.target.toleranceLufs);
  const withinTolerance = Math.abs(deltaFromTargetLufs) <= window;
  const ceilingDbtp = roundToMeasurementPrecision(result.target.truePeakCeilingDbtp);
  const exceedsCeiling =
    roundToMeasurementPrecision(result.metrics.truePeakDbtp) > ceilingDbtp;
  ...
  const projectedTruePeakDbtp = roundToMeasurementPrecision(
    result.metrics.projectedTruePeakDbtp ?? result.metrics.truePeakDbtp,
  );
  const projectedExceedsCeiling = projectedTruePeakDbtp > ceilingDbtp;
  ...
  if (projectedExceedsCeiling) {
    ...
    return {
      state: "ceiling-limited",
      label: "Ceiling-limited",
      description: `Integrated loudness is ${formatMeasurementNumber(Math.abs(deltaFromTargetLufs))} LU ${direction} target, and the ${move} needed to reach it still leaves the true peak above the selected ceiling.`,
      deltaFromTargetLufs,
    };
  }
  ...
  if (deltaFromTargetLufs < 0) {
    return {
      state: "below-target",
      label: "Needs gain",
      description: `Integrated loudness is ${formatMeasurementNumber(Math.abs(deltaFromTargetLufs))} LU below target.`,
      deltaFromTargetLufs,
    };
  }
  ...
  return {
    state: "above-target",
    label: "Too hot",
    description: `Integrated loudness is ${formatMeasurementNumber(deltaFromTargetLufs)} LU above target.`,
    deltaFromTargetLufs,
  };
}
